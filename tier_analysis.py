...
def target_by_attribute(G, attr, protected_countries=[]):
    ...
    def targeted(r, failure_scale="firm"):
        to_keep = sorted_attr_inds[failure_scale][
...
def failure_reachability(
    ...
    if parallel == "repeat" or parallel == "all":
        ...
        avgs = [
            failure_reachability_sweep(*args[0])
            for _ in tqdm.tqdm(range(repeats), desc="Reachability tier", leave=False)
        ]
    avgs = pd.concat(avgs, ignore_index=True)
...
def compare_tiers(
    ...
    for tiers in tqdm.tqdm(
        reversed(tier_range), total=len(tier_range), desc="Compare tiers"
    ):  # iterate over the number of tiers included
...
if __name__ == "__main__":
    ...
    if config["operations"]["get_thresholds"]:
        ...
        fname = (
            fname + "_" + sys.argv[1].replace(".xlsx", "") + "_" + start_time + ".xlsx"
        )
