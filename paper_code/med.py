...
def no_china_us_reachability(G, include_taiwan_hong_kong=False, prefix='.'):
    ...
    for x in G.es(tier=1):
        x.target_vertex['is_med'] = True
    ...
    def is_ch(x): return type(x['country'] == str) and (x['country'] in china)
    def is_us(x): return x['country'] == 'United States'
    ...
    reachable = pd.DataFrame([[cb(i.index, G, G_thin, tt, uu) for i, tt, uu in zip(
        med_suppliers, t, u)] for cb in callbacks], index=[cb.description for cb in callbacks]).transpose()
    ...
    by_country = reachable.groupby('country').mean().reindex(
        ['United States', 'China', 'Taiwan', 'Hong Kong'])

    os.makedirs(prefix + 'dat/', exist_ok=True)
    by_country.to_excel(prefix +
                        '/dat/no_us_china' +
                        ('_incl_taiwan_hk' if include_taiwan_hong_kong else '') +
                        '.xlsx')
...
def close_all_borders(G, prefix='.'):
    ...
    G_thin.delete_edges(
        G_thin.es.select(
            lambda e: e.source_vertex['country'] != e.target_vertex['country']))
...
def industry_deletion_effects(G, prefix='.'):
    ...
    for ind in set(G.vs['industry']):
        if not isinstance(ind, float):
            G_thin = deepcopy(G)
            G_thin.delete_vertices(G.vs(industry=ind))
            u = [get_u(i.index, G_thin) for i in med_suppliers]
            res[ind] = [np.mean([cb(None, G, G_thin, tt, uu)
                                for tt, uu in zip(t, u)]) for cb in callbacks]

    res = pd.DataFrame(
        res, index=[
            cb.description for cb in callbacks]).transpose()
...
def run_all_simulations(
    ...
    if (repeats is None) or (repeats == 'min'):
        ...
        for attack in attacks:
            for failure_scale in failure_scales:
                ...
                if (attack == random_thinning_factory) or (
                        attack == get_employee_attack):
                    repeats[attack, failure_scale] = max_repeats

    if rho_scales is None:
        # , np.linspace(.9,1,101)]#, np.linspace(.99,1,101),np.linspace(.999,1,101),np.linspace(.9999,1,101)]
        rho_scales = [full_rho]
    ...
    if scales_simulations:
        ...
        for rho in rho_scales:
            for failure_scale in failure_scales:
                for attack in attacks:
                    print(failure_scale + ' ' + (attack.description if attack else 'random') +
                          ' scale ' + str(rho[0]) + ' ' + str(rho[-1]))
                    plt.clf()
        ...
        for failure_scale in failure_scales:
            ...
            for rho in rho_scales:
                for metric in callbacks:
                    print(failure_scale + ' ' +
                          str(rho[0]) + ' ' + metric.description)
                    plt.clf()
                    attack_compare_plot(res_temp,
                                        failure_scale=failure_scale,
                                        rho_scale=rho,
                                        fname=prefix + 'im/attack_compare_' + str(rho[0]) + '_' + str(rho[-1]) + '/' + metric.description.replace(' ',
                                                                                                                                                  '_').replace('.',
                                                                                                                                                               '').lower() + '/' + failure_scale + '.svg',
                                        save=True,
    ...
    if software_compare:
        ...
        for G, inclusive in zip(graphs, (False, True)):
            for attack in attacks:
                print('Software ' +
                      ('included' if inclusive else 'excluded') +
                      ' ' +
                      (attack.description if attack else 'random'))
                plt.clf()
                avgs = failure_reachability(G,
                                            rho=full_rho,
                                            targeted_factory=attack,
                ...
                res = res.append(avgs, ignore_index=True)
        res.to_hdf(resfile, key='software', mode=(
            'a' if scales_simulations or tiers_simulations else write_mode))
        res['Software included'] = res['Software included'].astype(bool)
        to_return.append(res)

    if borders:
        print('no_china_us_reachability')
        no_china_us_reachability(
            G, include_taiwan_hong_kong=True, prefix=prefix)
        print('no_china_us_reachability')
        no_china_us_reachability(
            G, include_taiwan_hong_kong=False, prefix=prefix)
